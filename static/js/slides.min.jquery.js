...
(function(a) {
	a.fn.slides=function(b) {
		return b=a.extend({}, a.fn.slides.option, b), this.each(function() {
			function w(g, h, i) {
				if(!p&&o) {
					p=!0, b.animationStart(n+1);switch(g) {
				...
				o=!0, b.slidesLoaded();
			}), a('.'+b.next+', .'+b.prev).fadeOut(0), !1;if(e<2)return;h<0&&(h=0), h>e&&(h=e-1), b.start&&(n=h), b.randomize&&d.randomize(), a('.'+b.container, c).css({overflow: 'hidden', position: 'relative'}), d.children().css({position: 'absolute', top: 0, left: d.children().outerWidth(), zIndex: 0, display: 'none'}), d.css({position: 'relative', width: f*3, height: g, left: -f}), a('.'+b.container, c).css({display: 'block'}), b.autoHeight&&(d.children().css({height: 'auto'}), d.animate({height: d.children(':eq('+h+')').outerHeight()}, b.autoHeightSpeed));if(b.preload&&d.find('img:eq('+h+')').length) {
				a('.'+b.container, c).css({background: 'url('+b.preloadImage+') no-repeat 50% 50%'});let z=d.find('img:eq('+h+')').attr('src')+'?'+(new Date).getTime();a('img', c).parent().attr('class')!='slides_control'?t=d.children(':eq(0)')[0].tagName.toLowerCase():t=d.find('img:eq('+h+')'), d.find('img:eq('+h+')').attr('src', z).load(function() {
	...
	w('next', i);
		...
		function c() {
			...
			let d=a(this), e=d.children(), f=e.length;if(f>1) {
				e.hide();let g=[];for(i=0;i<f;i++)g[g.length]=i;g=g.sort(c), a.each(g, function(a, c) {
					let f=e.eq(c), g=f.clone(!0);g.show().appendTo(d), b!==undefined&&b(f, g), f.remove();
				});
			} 
		});
	};
