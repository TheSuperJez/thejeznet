...
$(document).ready(function () {
	...
	let navigation_links = $('#mainNav a');
	...
	sections.waypoint({
		handler: function (event, direction) {
			...
			let active_link = $('#mainNav a[href="#' + active_section.attr('id') + '"]');
	...
	$('.testimoniaContainer .testimonialContent .icoQuote').css({
		opacity: 0.2
	});
	$('.clientList a').css({
		opacity: 0.2
	...
	let $portfolioClone = $('.portfolio').clone();

	$('.filter a').click(function (e) {
		$('.filter li').removeClass('current');
		let $filterClass = $(this).parent().attr('class');
		if ($filterClass == 'all') {
