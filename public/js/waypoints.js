...
*/
(function($, k, m, i, d) {
	let e=$(i), g='waypoint.reached', b=function(o, n) {
			...
			$.extend(this, {element: $(n), oldScroll: 0, waypoints: [], didScroll: false, didResize: false, doScroll: $.proxy(function() {
				let q=this.element.scrollTop(), p=q>this.oldScroll, s=this, r=$.grep(this.waypoints, function(u, t) {
						return p?(u.offset>s.oldScroll&&u.offset<=q):(u.offset<=s.oldScroll&&u.offset>q); 
					}), o=r.length;if(!this.oldScroll||!q) {
						$[m]('refresh');
			...
			}, this)});$(n).bind('scroll.waypoints', $.proxy(function() {
				if(!this.didScroll) {
			...
			let o=null;$.each(f, function(p, q) {
				if(q.element[0]===n) {
					o=q;return false;
				} 
			});return o;
			...
			this.each(function() {
				let u=$.fn[k].defaults.context, q, t=$(this);if(n&&n.context) {
					u=n.context;
				}if(!$.isWindow(u)) {
				u=t.closest(u)[0]; 
			}q=j(u);if(!q) {
			q=new l(u);f.push(q);
		}let p=h(t, q), s=p<0?$.fn[k].defaults:q.waypoints[p].options, r=$.extend({}, s, n);r.offset=r.offset==='bottom-in-view'?function() {
			let v=$.isWindow(u)?$[m]('viewportHeight'):$(u).height();return v-$(this).outerHeight(); 
		}:r.offset;if(p<0) {
		...
		$.each(f, function(r, s) {
		let q=$.isWindow(s.element[0]), n=q?0:s.element.offset().top, p=q?$[m]('viewportHeight'):s.element.height(), o=q?0:s.element.scrollTop();$.each(s.waypoints, function(u, x) {
			if(!x) {
				return;
			}let t=x.options.offset, w=x.offset;if(typeof x.options.offset==='function') {
				t=x.options.offset.apply(x.element); 
			}else{
				if(typeof x.options.offset==='string') {
					let v=parseFloat(x.options.offset);t=x.options.offset.indexOf('%')?Math.ceil(p*(v/100)):v; 
				}
...
};$.fn[k].defaults={continuous: true, offset: 0, triggerOnce: false, context: i};$[m]=function(n) {
	if(a[n]) {
		return a[n].apply(this);
	}else{
		return a.aggregate();
	}
};$[m].settings={resizeThrottle: 200, scrollThrottle: 100};e.load(function() {
	$[m]('refresh'); 
}); 
